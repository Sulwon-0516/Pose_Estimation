...
def save_model(path, model, optimizer,scheduler,tot_loss,epoch,tot_EPOCH,num_tot_data,F_NAME,MODEL_NAME,is_best = False):
    if is_best:
...
def logger(log_string,config):
    ...
    file_path = os.path.join(log_path,config.LOG.FILE_NAME%(config.THEME,config.MODEL))
